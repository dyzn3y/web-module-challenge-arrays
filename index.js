...
function getRandomFlavors(originalFlavors, newFlavors, seasonalFlavors, regionalFlavors){
  const allFlavors = [];
  const thirtyOneFlavors = [];

  const pushFlavors = (allFlavorsArray, addFlavorsArray) => {
    addFlavorsArray.forEach(flavor => {
      allFlavorsArray.push(flavor);
    });
  };

  pushFlavors(allFlavors, originalFlavors);
  pushFlavors(allFlavors, newFlavors);
  pushFlavors(allFlavors, seasonalFlavors);
  pushFlavors(allFlavors, regionalFlavors);

  for (let i = 0; i < 31; i++) {
    thirtyOneFlavors.push(allFlavors[Math.floor(Math.random() * 101)])
  }
  // while(thirtyOneFlavors.length < 31) {
  //   
  // }
